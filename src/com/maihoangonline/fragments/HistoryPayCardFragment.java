...
public class HistoryPayCardFragment extends Fragment {

	private View rootView;
	private ListView lv;
	private ArrayList<Transaction> listPayCard = new ArrayList<Transaction>();
	private ListHistoryAdapter adapter;
	private ProgressDialog d;

	public static HistoryPayCardFragment INSTANCE;

	private HistoryPayCardFragment() {

	}

	public static HistoryPayCardFragment getInstance() {
		if (INSTANCE == null) {
			INSTANCE = new HistoryPayCardFragment();
		}
		return INSTANCE;
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		rootView = inflater.inflate(R.layout.fragment_pay_card, null);
		lv = (ListView) rootView.findViewById(R.id.history_pay_card);
		getHistory();
		return rootView;
	}

	private void getHistory() {
		DisplayUtils.log("PARAMS=1");
		listPayCard.clear();
			...
			public void onSuccess(int statusCode, Header[] headers,
				...
				try {
					...
					for (int i = 0; i < arr.length(); i++) {
						listPayCard.add(new Transaction(arr.getJSONObject(i)
		...
		ServiceConnection.getHistoryTransaction(
				SharePreferenceUtils.getAccID(getActivity()), 1, 0, handler);
	}
